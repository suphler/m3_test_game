...
var mainState = {
                    ...
                    for (var i = 0; i < 9; i++) {
                        for (var j = 0; j < 9; j++) {
                            if (field[i][j].img.markToDel) {
                                emptySet.push(field[i][j].img);
                                // field[i][j].img.angle -= 25;
                                field[i][j].img.kill();
                                // this.doGravity(); ////MEMORY LEAK!!!!
                            }
                        }
                    }
                    // console.log(emptySet);
                    // console.log('first dead: ');
                    // console.log(this.fieldGroup.getFirstDead());
                    resolve;
        ...
        doGravity: function () {
            ...
            for (var i = 0; i < 9; i++) {
                for (var j = 0; j < 9; j++) {
                    gravityStock.push(field[i][j].img);
                }
            }

            for (var i = 0; i < gravityStock.length; i++) {
                var gravityDownNeighbor = foundDownNeighbors(gravityStock[i]);
                // console.log("spot " + gravityStock[i].curXp + ',' + gravityStock[i].curYp + ' have DownNwighbor: ');
                // console.log(gravityDownNeighbor.curXp + ',' + gravityDownNeighbor.curYp);
                doGravityMove(gravityStock[i], gravityDownNeighbor);

            }

            function doGravityMove(spot, downNeighbor) {//TODO promisify!!


                if (spot.curYp != downNeighbor.curYp) {
                    console.log('try  to gravity  move: ' + spot.curXp + ',' + spot.curYp + 'to: ' + downNeighbor.curYp);
                    if (isInEmptySet(downNeighbor)) {
                        console.log('bottom spot is empty  - can  move!');
                        moveDown(spot, downNeighbor);
                    }

                }
            }

            function moveDown(spot, downSpot) {
                var tmpX, tmpY, tmpCurXp, tmpCurYp;
                ...
                spot.x = downSpot.x;
                spot.y = downSpot.y;
                spot.curXp = downSpot.curXp;
                spot.curYp = downSpot.curYp;
                for (var i = 0; i < emptySet.length; i++) {
                    if (spot.curXp == emptySet[i].curXp && spot.curYp == emptySet[i].curYp) {
                        emptySet[i].curXp = tmpCurXp;
                        emptySet[i].curYp = tmpCurYp;
                        emptySet[i].x = tmpX;
                        emptySet[i].y = tmpY;
                    }
                }
                ...
                if (spot.curYp != foundDownNeighbors(spot).curYp){

                    // doGravityMove(spot, foundDownNeighbors(spot));
                    // this.doGravity();
                }
            ...
            function isInEmptySet(spot) {

                for (var i = 0; i < emptySet.length; i++) {
                    // console.log('check for alive: ' + spot.curXp + ',' + spot.curYp + '-- ' + emptySet[i].curXp + ',' + emptySet[i].curYp);
                    if (spot.curXp == emptySet[i].curXp && spot.curYp == emptySet[i].curYp) {
            ...
            function foundDownNeighbors(spot) {
                // var neighbors = [];
                var down;
                for (var i = 0; i < gravityStock.length; i++) {
                    if (spot.curYp == gravityStock[i].curYp - 1 && spot.curXp == gravityStock[i].curXp) {
                        down = gravityStock[i];
  //                      console.log('down spot is alive !')
                        return down;
                    }
                }
                for (var j = 0; j < emptySet.length; j++) {
                    if (spot.curYp == emptySet[j].curYp - 1 && spot.curXp == emptySet[j].curXp) {
                        down = emptySet[j];
                        console.log('I found  it in  an  empty set !')
                        return down;
                    }
        ...
        buildFild: function () {
            ...
            for (var i = 0; i < 9; i++) {
                ...
                for (var j = 0; j < 9; j++) {
                    curX += 110;
                    field[i][j] = {
                        img: game.add.sprite(30 + curX, 30 + curY, 'img0' + this.getRndImgN(), 0, this.fieldGroup),
                    ...
                    this.fieldGroup.add(field[i][j].img);
                    curXp++;
                    // console.log(field);
                }
                curYp++;
        ...
        doGoalCheckGraph: function () {
            ...
            function buildGraphTree(spot) { //TODO  need to  add check  iff  neighbor  is alive!!!
            ...
            this.marckToDel(marckToDel).then(
                resolve => {
                    console.log("RESOLVED!!");
                    this.delMarcked();
                    console.log("empty set  cosist of: ");
                    console.log(emptySet);

                }
            ).then(
                resolve=>{
                    console.log("RESOLVED!! do gravity");
                    this.doGravity();
                }
            );
        ...
        marckToDel: function (arrToDel) {
            console.log("marcking to del in  promise ")
            return new Promise(
                function (resolve, reject) {
